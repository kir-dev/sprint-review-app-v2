...
export function WorkPeriodProgress({
  currentPeriod,
  loading,
}: WorkPeriodProgressProps) {
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex justify-between mt-1 text-xs text-muted-foreground">
          <span>
            {new Date(currentPeriod.startDate).toLocaleDateString('hu-HU')}
          </span>
          <span>{Math.round(progress)}%</span>
          <span>
            {new Date(currentPeriod.endDate).toLocaleDateString('hu-HU')}
          </span>
        </div>
      </CardContent>
    </Card>
