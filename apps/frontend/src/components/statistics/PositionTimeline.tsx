...
export function PositionTimeline({
  ...
  history,
  className,
}: PositionTimelineProps & { className?: string }) {
            ...
            {sortedHistory.map((item, index) => {
              ...
              return (
                <div key={item.id} className="relative pl-8">
                  {/* Timeline dot */}
                  <div
                    className={`absolute -left-[5px] top-1 h-2.5 w-2.5 rounded-full border border-background ${isCurrent ? 'bg-primary' : 'bg-muted-foreground'}`}
                  />
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="text-sm text-muted-foreground flex items-center gap-2">
                      ...
                      <span>
                        {format(new Date(item.startDate), 'yyyy. MMM', {
                          locale: hu,
                        })}
                        {' - '}
                        {item.endDate
                          ? format(new Date(item.endDate), 'yyyy. MMM', {
                              locale: hu,
                            })
                          : 'Jelenleg'}
                      </span>
