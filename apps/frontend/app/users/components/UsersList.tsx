...
interface UsersListProps {
  users: User[];
  isLoading: boolean;
  onPositionChange: (userId: number, newPosition: string) => Promise<void>;
  currentUser: {
    id: number;
    position: string;
    positionDetails?: {
...
export function UsersList({
  ...
  const sortedUsers = useMemo(() => {
    return [...users].sort((a, b) => {
      const aIndex = positions.findIndex((p) => p.name === a.position);
      const bIndex = positions.findIndex((p) => p.name === b.position);
      // If a position is not in the list, place it at the end
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {sortedUsers.map((user, index) => {
        const userPositionDetails = user.positionDetails || positions.find((p) => p.name === user.position);
        const positionLabel = userPositionDetails?.label || user.position;
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <button
                    ...
                    className={cn(
                      'w-full flex items-center justify-between gap-2 px-3 py-2 rounded-lg border text-sm font-medium transition-all bg-card hover:bg-accent/50',
